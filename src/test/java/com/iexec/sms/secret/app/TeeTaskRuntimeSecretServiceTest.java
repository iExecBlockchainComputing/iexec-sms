...
class TeeTaskRuntimeSecretServiceTest {
    ...
    void shouldAddSecret() {
        doReturn(false).when(teeTaskRuntimeSecretService)
                .isSecretPresent(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0);
        when(encryptionService.encrypt(DECRYPTED_SECRET_VALUE))
                .thenReturn(ENCRYPTED_SECRET_VALUE);

        teeTaskRuntimeSecretService.encryptAndSaveSecret(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0, DECRYPTED_SECRET_VALUE);
    ...
    void shouldGetSecret() {
        ...
        Optional<TeeTaskRuntimeSecret> encryptedSecret = teeTaskRuntimeSecretService.getSecret(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0, false);
        ...
        Optional<TeeTaskRuntimeSecret> decryptedSecret = teeTaskRuntimeSecretService.getSecret(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0, true);
    ...
    void secretShouldExist() {
        when(teeTaskRuntimeSecretService.getSecret(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0, false))
                .thenReturn(Optional.of(RUNTIME_SECRET));

        final boolean isSecretPresent = teeTaskRuntimeSecretService.isSecretPresent(DeployedObjectType.APPLICATION, APP_ADDRESS, SecretOwnerRole.APPLICATION_DEVELOPER, null, 0);
